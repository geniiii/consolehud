...
public class RenderPaperDoll implements ConsoleHudRender {
	...
	private void onClientTick() {
		if (ConsoleHud.CLIENT.player != null && !ConsoleHud.CLIENT.isPaused() && ConsoleHud.CONFIG.paperDoll) {
			final boolean sprinting = ConsoleHud.CLIENT.player.isSprinting() && ConsoleHud.CONFIG.paperDollConfig.sprinting;
			final boolean crouching = ConsoleHud.CLIENT.player.isSneaking() && remainingRidingTicks == 0 && ConsoleHud.CONFIG.paperDollConfig.crouching;
			final boolean flying = ConsoleHud.CLIENT.player.abilities.flying && ConsoleHud.CONFIG.paperDollConfig.flying;
			final boolean elytra = ConsoleHud.CLIENT.player.isFallFlying() && ConsoleHud.CONFIG.paperDollConfig.elytraFlying;
			final boolean burning = ConsoleHud.CLIENT.player.isOnFire() && ConsoleHud.CONFIG.paperDollConfig.burning;
			final boolean mounting = ConsoleHud.CLIENT.player.isRiding() && ConsoleHud.CONFIG.paperDollConfig.riding;
			final boolean swimmingPose = ConsoleHud.CLIENT.player.isInSwimmingPose() && ConsoleHud.CONFIG.paperDollConfig.swimmingPose;

			if (ConsoleHud.CONFIG.paperDollConfig.always || crouching || sprinting || burning || elytra || flying || mounting || swimmingPose) {
				remainingTicks = 20;
